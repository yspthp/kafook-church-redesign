document.addEventListener('DOMContentLoaded', () => {
  ...
  if (header && menu && !button) {
    ...
    button.id = 'menuToggle'; button.type = 'button'; button.className = 'mobile-menu-toggle';
  ...
  if (header && button && menu) {
    if (!menu.querySelector('a[href="contact.html"]')) {
      ...
      const link = document.createElement('a'); link.className = 'nav-link'; link.href = 'contact.html'; link.textContent = '聯絡我們';
      item.append(link); menu.append(item);
    }
    menu.setAttribute('aria-label', '主要導覽'); header.classList.add('menu-ready'); button.setAttribute('aria-controls', 'navMenu');
    ...
    document.addEventListener('keydown', e => { if (e.key === 'Escape' && menu.classList.contains('active')) { setOpen(false); button.focus(); } });
    document.addEventListener('click', e => { if (!header.contains(e.target)) setOpen(false); });
  }
  if (header) { const update = () => header.classList.toggle('scrolled', scrollY > 20); addEventListener('scroll', update, {passive:true}); update(); }
  document.querySelectorAll('a[href^="#"]').forEach(anchor => anchor.addEventListener('click', e => {
    const id = anchor.getAttribute('href').slice(1), target = document.getElementById(id); if (!target) return; e.preventDefault();
  ...
  if (!scripts.some(src => src.endsWith('/cms.js'))) {
    if (!document.querySelector('link[href*="cms.css"]')) { const css = document.createElement('link'); css.rel = 'stylesheet'; css.href = 'css/cms.css'; document.head.append(css); }
    const config = document.createElement('script'); config.src = 'js/supabase-config.js'; document.head.append(config);
    config.onload = () => { const cms = document.createElement('script'); cms.src = 'js/cms.js'; document.body.append(cms); };
  }
});
